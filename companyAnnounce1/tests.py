...
from .models import Announcement1


class AnnouncementAdminTest(APITestCase):
    def setUp(self):
        self.user = self.create_super_admin()
        self.url = self.reverse("company_announcement_admin_api")
        print("notification admin url: ", self.url)
    ...
    def test_delete_announcement(self):
        ...
        self.assertSuccess(resp)
        self.assertFalse(Announcement1.objects.filter(id=id).exists())


class AnnouncementAPITest(APITestCase):
    def setUp(self):
        self.user = self.create_super_admin()
        Announcement1.objects.create(title="title", content="content", visible=True, created_by=self.user)
        self.url = self.reverse("company_announcement_api")
        print("notification oj url: ", self.url)
