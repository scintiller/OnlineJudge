...
from .models import Announcement2


class AnnouncementAdminTest(APITestCase):
    def setUp(self):
        self.user = self.create_super_admin()
        self.url = self.reverse("company_notification_admin_api")
    ...
    def test_delete_announcement(self):
        ...
        self.assertSuccess(resp)
        self.assertFalse(Announcement2.objects.filter(id=id).exists())


class AnnouncementAPITest(APITestCase):
    def setUp(self):
        self.user = self.create_super_admin()
        Announcement2.objects.create(title="title", content="content", visible=True, created_by=self.user)
        self.url = self.reverse("company_notification_api")
